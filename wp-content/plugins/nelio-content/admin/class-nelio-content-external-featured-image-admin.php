...
class Nelio_Content_External_Featured_Image_Admin {
	...
	public function init() {
		...
		add_action( 'nelio_content_site_created', [ $this, 'set_efi_mode' ] );
		add_filter( 'admin_init', [ $this, 'disable_original_nelioefi_hooks' ], 1 );

		add_action( 'after_switch_theme', [ $this, 'set_efi_mode' ] );
		add_action( 'after_switch_theme', [ $this, 'maybe_regenerate_efi_placeholder_thumbnails' ] );

		add_action( 'plugins_loaded', [ $this, 'add_settings_based_hooks' ] );
	...
	public function add_settings_based_hooks() {
		...
		add_action( 'save_post', [ $aux, 'extract_featured_images_for_autoset' ] );
	...
	public function set_efi_mode() {
		...
		$settings        = Nelio_Content_Settings::instance();
		$aux             = get_option( $settings->get_name(), [] );
