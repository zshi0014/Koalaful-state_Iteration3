...
class Nelio_Content_Ics_Calendar {
	...
	public function init() {
		...
		add_action( 'plugins_loaded', [ $this, 'do_init' ] );
	...
	public function do_init() {
		...
		add_action( 'wp_ajax_nelio_content_calendar_ics_subscription', [ $this, 'handle_ics_subscription' ] );
		add_action( 'wp_ajax_nopriv_nelio_content_calendar_ics_subscription', [ $this, 'handle_ics_subscription' ] );
	...
	public function handle_ics_subscription() {
		 ...
		 * @since 2.0.0
		 */
		$this->start_date = apply_filters( 'nelio_content_ics_calendar_subscription_start_date', $this->get_beginning_of_week( date( 'Y-m-d', current_time( 'timestamp' ) ) ) );
		...
		// Fix filters.
		add_filter( 'the_title', [ $this, 'do_ics_escaping' ], 99 );
			...
			foreach ( $week_posts as $date => $day_posts ) {
				foreach ( $day_posts as $num => $post ) {
					...
					$formatted_post = array(
						'BEGIN'         => 'VEVENT',
						'UID'           => $post->guid,
						'SUMMARY'       => apply_filters( 'the_title', $post->post_title, $post->ID ) . ' - ' . $status_helper->get_post_status_friendly_name( get_post_status( $post->ID ) ),
						'DTSTAMP'       => $start_date,
						'DTSTART'       => $start_date,
						'DTEND'         => $end_date,
		...
		$header = array(
			'BEGIN'   => 'VCALENDAR',
			'VERSION' => '2.0',
			'PRODID'  => '-//Nelio Content//Nelio Content ' . $aux->get_version() . '//EN',
		);
	 ...
	 * @access public
	 */
	private function get_calendar_posts_for_week( $args = [] ) {
