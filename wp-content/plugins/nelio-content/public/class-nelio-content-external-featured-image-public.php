...
class Nelio_Content_External_Featured_Image_Public {
	...
	public function add_hooks() {
		...
		// Disable old hooks.
		add_filter( 'init', [ $this, 'disable_original_nelioefi_hooks' ], 1 );

		// Featured Image Hooks.
		add_filter( 'init', [ $this, 'add_hooks_for_thumbnail_simulation' ] );
		add_filter( 'wp_get_attachment_image_src', [ $this, 'maybe_return_efi_url' ], 10, 3 );
		add_filter( 'post_thumbnail_html', [ $this, 'maybe_add_efi_as_background' ], 10, 5 );

		// Fix og:image meta tag.
		add_action( 'wp_head', [ $this, 'leave_wp_head_section' ], 99999 );
	...
	public function add_hooks_for_thumbnail_simulation() {
		...
		foreach ( get_post_types() as $post_type ) {
			...
			add_filter( "get_${post_type}_metadata", [ $this, 'maybe_simulate_post_thumbnail_attachment' ], 10, 3 );
	...
	public function remove_hooks_for_thumbnail_simulation() {
		...
		foreach ( get_post_types() as $post_type ) {
			remove_filter( "get_${post_type}_metadata", [ $this, 'maybe_simulate_post_thumbnail_attachment' ], 10, 3 );
		}//end foreach
	...
	public function maybe_simulate_post_thumbnail_attachment( $null, $object_id, $meta_key ) {
		...
		// If there's a regular featured image, keep going.
	 ...
	 * @access public
	 */
	public function maybe_add_efi_as_background( $html, $post_id = 0, $post_thumbnail_id = '', $size = false, $attr = [] ) {
