...
class Nelio_Content_Notifications {
	...
	public function init() {
		...
		add_action( 'plugins_loaded', [ $this, 'add_hooks_if_notifications_are_enabled' ] );
		add_action( 'delete_user', [ $this, 'delete_follower' ] );
	...
	public function add_hooks_if_notifications_are_enabled() {
		...
		add_action( 'nelio_content_notify_post_followers', [ $this, 'maybe_notify_post_followers' ], 10, 4 );

		// Editorial comments change actions.
		add_action( 'nelio_content_after_create_editorial_comment', [ $this, 'maybe_send_comment_creation_notification' ] );

		// Editorial tasks change actions.
		add_action( 'nelio_content_after_create_editorial_task', [ $this, 'maybe_send_task_creation_notification' ] );
		add_action( 'nelio_content_after_update_editorial_task', [ $this, 'maybe_send_task_status_change_notification' ] );
	...
	public function maybe_notify_post_followers( $post_id, $followers, $old_status, $old_followers ) {
		 ...
		 * @since 1.4.2
		 */
		$ignored_statuses = apply_filters( 'nelio_content_notification_ignored_statuses', [ 'inherit', 'auto-draft' ], $post->post_type );
		$ignored_statuses = array_merge( $ignored_statuses, [ $old_status ] );
	...
	public function maybe_send_comment_creation_notification( $comment ) {
		...
		$recipients = array_values( array_unique( array_merge( $followers, [ $comment['author'] ] ) ) );
	...
	public function maybe_send_task_creation_notification( $task ) {
		...
		$helper     = Nelio_Content_Post_Helper::instance();
		$followers  = $helper->get_post_followers( $comment['post'] );
		$recipients = array_values( array_unique( array_merge( $followers, [ $task['assignerId'], $task['assigneeId'] ] ) ) );
	...
	public function delete_follower( $id ) {
		...
		return $wpdb->delete( // phpcs:ignore
			$wpdb->postmeta,
			array(
				'meta_key'   => '_nc_following_users', // slow query ok
				'meta_value' => $id,                   // slow query ok
			)
		);
