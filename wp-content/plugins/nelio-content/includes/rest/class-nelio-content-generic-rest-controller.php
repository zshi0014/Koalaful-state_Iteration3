...
class Nelio_Content_Generic_REST_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'callback'            => [ $this, 'reset_auto_social_messages' ],
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::EDITABLE,
					'callback'            => [ $this, 'pause_publication' ],
					'permission_callback' => 'nc_can_current_user_manage_plugin',
					'args'                => array(
						'paused' => array(
							'required'          => true,
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::EDITABLE,
					'callback'            => [ $this, 'run_new_comment_action' ],
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::EDITABLE,
					'callback'            => [ $this, 'run_update_task_action' ],
					'permission_callback' => 'nc_can_current_user_use_plugin',
					'args'                => array(
						'assigneeId' => array(
							'required'          => true,
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::EDITABLE,
					'callback'            => [ $this, 'clean_plugin' ],
					'permission_callback' => [ $this, 'check_if_user_can_deactivate_plugin' ],
					'args'                => array(
						'_nonce' => array(
							'required'          => true,
							'type'              => 'string',
							'validate_callback' => nc_compose( 'trim', 'nc_is_not_empty' ),
							'sanitize_callback' => nc_compose( 'sanitize_text_field', 'trim' ),
						),
					),
				),
	...
	public function pause_publication( $request ) {
		...
		$is_paused = $result->isMessagePublicationPaused;
	...
	/**
	 * Runs an action so that post followers can be notifiedw when a new comment has been added to a post.
	 *
	 * @param WP_REST_Request $request Full data about the request.
	 *
	 * @return WP_REST_Response The response.
	...
	public function clean_plugin( $request ) {
		...
		$data = array(
			...
			'body'      => wp_json_encode( [ 'reason' => $request['reason'] ?: 'none' ] ),
			'headers'   => array(
				'Authorization' => 'Bearer ' . nc_generate_api_auth_token(),
				'accept'        => 'application/json',
				'content-type'  => 'application/json',
